...
function Initialize-Screens {
    try {
        Write-Log "Initializing application screens"
        
        # Initialize screens dictionary if it doesn't exist
        if ($null -eq $script:screens) {
            $script:screens = @{}
        }
        
        # Create and store screen objects
        $screenNames = @('SoftAppsScreen', 'OptimizeScreen', 'CustomizeScreen', 'AboutScreen')
        
        foreach ($screenName in $screenNames) {
            $screen = $window.FindName($screenName)
            if ($null -eq $screen) {
                Write-Log "Warning: Screen '$screenName' not found in XAML" -Severity WARNING
                continue
            }
            $script:screens[$screenName] = $screen
            $screen.Visibility = 'Collapsed'
        }
        
        # Set initial screen
        if ($script:screens.ContainsKey('SoftAppsScreen')) {
            $script:screens['SoftAppsScreen'].Visibility = 'Visible'
            $SoftwareAppsNavButton.IsChecked = $true
        }
        else {
            Write-Log "Error: Initial screen 'SoftAppsScreen' not available" -Severity ERROR
        }
    }
    catch {
...
function Initialize-PackageStatusCache {
    try {
        ...
        $SCRIPT:InstalledCapabilities = Get-WindowsCapability -Online | Where-Object { $_.State -eq "Installed" }
        Write-Log "[INFO] Cached $($SCRIPT:InstalledCapabilities.Count) capabilities"
        
        Write-Log "[INFO] Caching installed packages"
        # Check if we're on Windows 10 and handle Appx module differently
        if ([System.Environment]::OSVersion.Version.Major -eq 10) {
            try {
                # Try using Windows PowerShell for Appx commands
                $SCRIPT:InstalledPackages = powershell.exe -Command {
                    Get-AppxPackage | Select-Object Name, PackageFullName
                }
            }
            catch {
                Write-Log "[WARNING] Unable to get Appx packages, some features may be limited: $_"
                $SCRIPT:InstalledPackages = @()
            }
        }
        else {
            $SCRIPT:InstalledPackages = Get-AppxPackage
        }
        Write-Log "[INFO] Cached $($SCRIPT:InstalledPackages.Count) packages"
    }
    catch {
        Write-Log "[ERROR] Failed to initialize package status cache: $_"
        # Don't throw the error, just continue with empty arrays
        $SCRIPT:InstalledCapabilities = @()
        $SCRIPT:InstalledPackages = @()
    }
}
...
try {
    ...
    try {
        ...
        Write-Log "Failed to initialize screens: $_" -Severity ERROR
    ...
    try {
        ...
        Write-Log "Failed to initialize installation handlers: $_" -Severity ERROR
        $initializationSuccess = $false
    }
    ...
    try {
        ...
        Write-Log "Failed to initialize bloatware UI: $_" -Severity ERROR
        $initializationSuccess = $false
    }
    ...
    try {
        ...
        Write-Log "Failed to initialize app theme: $_" -Severity ERROR
        $initializationSuccess = $false
    }
    ...
    if (-not $initializationSuccess) {
        Show-MessageBox -Message "Some features may be limited due to initialization errors. Check the log file for details." -Title "Initialization Warning" -Icon Warning
    }
